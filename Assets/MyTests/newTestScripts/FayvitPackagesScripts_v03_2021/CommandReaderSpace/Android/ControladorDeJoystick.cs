...
#pragma warning restore 0649
        ...
        private void Start()
        {
            ...
            MessageAgregator<RequestHideControllersMessage>.AddListener(OnRequestHideControlls);
            MessageAgregator<RequestShowControllsMessage>.AddListener(OnRequestShowControlls);
            MessageAgregator<ChangeHardwareControllerMessage>.AddListener(OnChangeHardwareController);
        ...
        {
            MessageAgregator<RequestHideControllersMessage>.RemoveListener(OnRequestHideControlls);
            MessageAgregator<RequestShowControllsMessage>.RemoveListener(OnRequestShowControlls);
            MessageAgregator<ChangeHardwareControllerMessage>.RemoveListener(OnChangeHardwareController);
        ...
        private void OnChangeHardwareController(ChangeHardwareControllerMessage obj)
        {
            transform.parent.gameObject.SetActive(obj.controlador == Controlador.Android);
        }

        private void OnRequestShowControlls(RequestShowControllsMessage obj)
        {
            Controlador c = obj.controlador;
            if (c == Controlador.Android)
                transform.parent.gameObject.SetActive(true);
        }

        private void OnRequestHideControlls(RequestHideControllersMessage obj)
        {
            if (moveJoy != null)
